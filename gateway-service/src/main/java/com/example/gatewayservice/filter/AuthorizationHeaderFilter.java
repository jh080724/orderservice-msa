...
public class AuthorizationHeaderFilter
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> { // exchange: 요청과 응답을 가지고 있는 객체, chain: 필터에게 전달?
            ...
            // JWT 토큰 유효성 검증 및 클레임(Claim) 얻어내기
            ...
            // 위에서 exchange를 변화시켰으니, 다시 변화(mutate)시켜서 전달해야 한다.
            // 새롭게 만든(토큰 정보를 헤더에 담은) request를 exchange에 갈아 끼워서 보내자.
    ...
    // Spring Webflux에서 사용하는 타입 Mono, Flux가 항상 같이 다님.
    // Mono: 단일 값 또는 완료 신호 등을 처리
    // Flux: 여러 데이터 블록, 스트림을 처리
    // API gateway는 톰캣이 아닌 Netty를 사용함. 비동기 처리
    // request, response를 바롤 사용하지 않고, Mono, Flux를 사용하는 이유는 게이트웨이 서버가
