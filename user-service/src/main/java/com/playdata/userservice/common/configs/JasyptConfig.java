...
public class JasyptConfig {
    ...
    public StringEncryptor getStringEncryptor() {
        ...
        // 핵심객체의 설정값을 객체 형태로 한번에 모아서 (encryptor)에게 전달할 수 있게 하는 객체
        ...
        config.setPoolSize("1");    // 암호화에 사용할 쓰래드 개수(값을 크게주면 암호화 성능 향상)
        config.setProviderName("SunJCE");   // 암호화 제공가
        config.setStringOutputType("base64");   // 암호화된 문자열의 출력 형식
        //  Salt: 암호화된 데이터를 보호하기 위해 암호화 과정에서 추가되는 랜덤값
        config.setSaltGeneratorClassName("org.jasypt.salt.RandomSaltGenerator");
        // IV(Initialization Vector) : 암호화 과정에서 초기값으로 사용.
